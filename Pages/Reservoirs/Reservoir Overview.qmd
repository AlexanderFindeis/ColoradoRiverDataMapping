...
```{ojs}
render_date = d3.max(
  Res_Elv.filter(d => d.Type === "Recorded"),
  d => d.Date).toLocaleDateString("en-US", {
    month: "long",
    day: "numeric",
    year: "numeric"
})
...
elv_data_subset = filtered_elv.filter(d => d.Type === "Recorded")
elv_stats = {
  const target      = elv_data_subset.find(d => 
    d.Date.toISOString().slice(0, 10) === "2000-01-01"
  ) 
  const elv_2000    = target?.Elevation ?? "N/A"
  const Date = elv_data_subset[elv_data_subset.length - 1]?.Date
    ? elv_data_subset[elv_data_subset.length - 1].Date.toLocaleDateString("en-US", { month: "long", day: "numeric" }): "N/A"
  const last        = elv_data_subset[elv_data_subset.length - 1]
...
//| echo: false
{
  ...
  const marks = [
    Plot.lineY(filtered_elv, { x: "Date", y: "Elevation", stroke: "Type" }),
  ...
  const Elevation_Chart = Plot.plot({
    ...
    color: {
      legend: true,
      domain: ["Recorded", "Most Probable", "Minimum Probable", "Maximum Probable"],
...
```{ojs}
// Summary stats from filtered data
stor_data_subset = filtered_stor.filter(d => d.Type === "Recorded")
stor_stats = {
  const target      = stor_data_subset.find(d => 
    d.Date.toISOString().slice(0, 10) === "2000-02-06"
  ) 
  const stor_2000   = target?.Storage_MAF ?? "N/A"
  const Date = stor_data_subset[stor_data_subset.length - 1]?.Date
    ? stor_data_subset[stor_data_subset.length - 1].Date.toLocaleDateString("en-US", { month: "long", day: "numeric" }): "N/A"
  const last        = stor_data_subset[stor_data_subset.length - 1]
...
">
  ...
  <div style="
    ...
    padding: 16px;
  ">
    <div style="font-size: 12px; color: #6c757d; text-transform: uppercase; letter-spacing: 0.05em;">Storage ${stor_stats.Date} Last Year</div>
...
//| echo: false
{
  ...
  const Storage_Chart = Plot.plot({
    ...
    marks: [
      Plot.lineY(filtered_stor, { x: "Date", y: "Storage_MAF", stroke: "Type" }),
         // Add a ruleY and text annotation for each reference line
...
">
  <div>
    <label style="font-size:12px; font-weight:600; color:#6c757d; 
                  text-transform:uppercase; display:block; margin-bottom:4px;">
      Year A
    </label>
    <select id="year-a" style="
      padding: 6px 12px;
      border: 1px solid #dee2e6;
      border-radius: 6px;
      font-size: 13px;
  ...
  <div style="margin-top:16px; display:flex; gap:16px; font-size:12px;">
    <div style="display:flex; align-items:center; gap:6px;">
      <div style="width:14px; height:14px; background:#5bc0de; 
                  opacity:0.7; border-radius:2px;"></div>
      <span id="legend-a">Year A</span>
    </div>
    <div style="display:flex; align-items:center; gap:6px;">
      <div style="width:14px; height:14px; background:#da3030; 
                  opacity:0.7; border-radius:2px;"></div>
      <span id="legend-b">Year B</span>
    </div>
  </div>
</div>

<!-- Map container -->
<div id="map" style="width:100%; height:650px; border-radius:8px;"></div>
```

```{=html}
<script>
  // --- Populate year selectors ---
  const years     = Array.from({length: 2027 - 1984}, (_, i) => 1984 + i)
  ...
  // Reservoir center and zoom lookup
  const reservoir_locations = {
    "Total":                  { center: [-112.0, 36.5], zoom: 5  },
    "Lake Mead":              { center: [-114.425, 36.154], zoom: 10  },
    "Lake Powell":            { center: [-110.6, 37.4], zoom: 8  },
    "Flaming Gorge":          { center: [-109.548, 41.151], zoom: 9  },
    "Lake Mohave":            { center: [-114.643, 35.425],  zoom: 9  },
    "Navajo Reservoir":       { center: [-107.5, 36.92],  zoom: 10 },
    "Strawberry Reservoir":   { center: [-111.16, 40.18], zoom: 10 },
    "Blue Mesa Reservoir":    { center: [-107.132, 38.467], zoom: 11 },
    "Lake Havasu":            { center: [-114.26, 34.397], zoom: 10 },
    "Granby Reservoir":       { center: [-105.83, 40.14], zoom: 11 },
    "McPhee Reservoir":       { center: [-108.545, 37.516], zoom: 11 },
    "Fontenelle Reservoir":   { center: [-110.08, 42.05], zoom: 11 },
    "Starvation Reservoir":   { center: [-110.45, 40.2],  zoom: 11 },
    "Green Mountain Reservoir":{ center: [-106.28, 39.87], zoom: 12 },
    "Vallecito Reservoir":    { center: [-107.55, 37.39], zoom: 12 },
    "Lake Nighthorse":        { center: [-107.9, 37.224], zoom: 12 },
    "Morrow Point Reservoir": { center: [-107.48, 38.44], zoom: 12 },
    "Taylor Park Reservoir":  { center: [-106.582, 38.83], zoom: 12 },
    "Ruedi Reservoir":        { center: [-106.78, 39.36], zoom: 12 }
  }

  // Listen for reservoir change events from OJS
  window.addEventListener("reservoir-change", (e) => {
    const selected = e.detail.reservoir
    const location = reservoir_locations[selected]

    if (location && map.loaded()) {
      map.flyTo({
  ...
  function update_layers(year_a, year_b) {

    const path_a = `/GIS_Data/Surface_Water/SW_${year_a}.geojson`
    const path_b = `/GIS_Data/Surface_Water/SW_${year_b}.geojson`

    // Update legend labels
    legendA.textContent = `Year A: ${year_a}`
    legendB.textContent = `Year B: ${year_b}`

    // --- Update or add Year A source and layers ---
    if (map.getSource("layer-a")) {
      // Source exists - just update the data
      map.getSource("layer-a").setData(path_a)
    } else {
      // First load - add source and layers
      map.addSource("layer-a", {
        type: "geojson",
        data: path_a
      })
    ...
    // --- Update or add Year B source and layers ---
    if (map.getSource("layer-b")) {
      map.getSource("layer-b").setData(path_b)
    } else {
      map.addSource("layer-b", {
        type: "geojson",
        data: path_b
      })
  ...
  // --- Load initial layers when map is ready ---
  map.on("load", () => {
    update_layers(selectA.value, selectB.value)
  })

  // --- Update layers when new dates are selected ---
  selectA.addEventListener("change", () => {
    if (map.loaded()) update_layers(selectA.value, selectB.value)
  })
  ...
  // --- Popups ---
  // const add_popup = (layer_id, year_label) => {
  //   map.on("click", layer_id, (e) => {
  //     const props = e.features[0].properties
  //     new maplibregl.Popup()
  //       .setLngLat(e.lngLat)
  //       .setHTML(`
  //         <b>${year_label}: ${props.value === 3 ? "Permanent Water" : "Water"}</b>
  //       `)
  //       .addTo(map)
  //   })
  ...
  // map.on("load", () => {
  //   add_popup("fill-a", "Year A")
  //   add_popup("fill-b", "Year B")
  // })
</script>
```
::: chart-description
Surface water extent data is from the JRC Global Surface Water Dataset, using satellite data to map surface water from 1984 to 2020. Years after 2020 are created using data from the Sentinel-2 satellite through the OpenEO API. Normalized Difference Water Index was calculated and vectorized to create polygons of surface water extent for included reservoirs, using the median value for each pixel across the entire year. The current year is the median of the past two months to approximate current levels and account for discrepencies and cloud cover in individual collections.
:::
:::
:::::
<br>
