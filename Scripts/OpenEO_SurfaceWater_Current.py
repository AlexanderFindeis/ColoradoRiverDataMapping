...
Past_60Days  = (date.today() - timedelta(days=60)).strftime("%Y-%m-%d")

print(f"Temporal extent: {Past_60Days} to {Current_Date}")
...
cube = connection.load_collection(
    ...
    temporal_extent = [Past_60Days, Current_Date],
    bands           = ["B03", "B08"],
    max_cloud_cover = 1
)
...
if results:
    ...
    water_gdf.to_file("GIS_Data/Surface_Water/SW_2026.geojson", driver="GeoJSON")
    print(f"Saved: GIS_Data/Surface_Water/SW_2026.geojson ({len(water_gdf)} features)")
else:
